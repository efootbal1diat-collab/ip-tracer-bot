...
<body class="min-h-screen pb-16">
                        ...
                        <form method="GET" action="/">
                            <select name="subnet" onchange="this.form.submit()" class="text-xs bg-blue-100 text-blue-800 font-medium px-2.5 py-1 rounded-full border border-blue-200 focus:outline-none cursor-pointer">
                                @foreach($subnets as $s)
                                    <option value="{{ $s }}" {{ $activeSubnet === $s ? 'selected' : '' }}>Subnet {{ $s }}.xxx</option>
                                @endforeach
                            </select>
                        </form>
                    </h1>
                    <p class="text-xs text-gray-500">
                        Last scan: <span class="font-semibold text-gray-700">{{ $lastScanTime }}</span>
    ...
    <main class="max-w-7xl mx-auto px-6 mt-6 space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
            ...
            <div class="google-card p-4 flex items-center space-x-4">
                ...
                <div>
                    <div class="text-2xl font-bold text-green-700" id="statActive">{{ $summaryStats['active_count'] }}</div>
            ...
            <div class="google-card p-4 flex items-center space-x-4">
                ...
                <div>
                    <div class="text-2xl font-bold text-amber-700">{{ $summaryStats['unmapped_active'] }}</div>
            ...
            <div class="google-card p-4 flex items-center space-x-4">
                ...
                <div>
                    <div class="text-2xl font-bold text-indigo-600">{{ $summaryStats['excel_mapped'] }}</div>
            ...
            <div class="google-card p-4 flex items-center space-x-4">
                ...
                <div>
                    <div class="text-2xl font-bold text-gray-600">{{ $summaryStats['free_count'] }}</div>
        ...
        <div id="scanProgressContainer" class="hidden google-card p-4 border border-blue-200 bg-blue-50/50">
            <div class="flex items-center justify-between text-sm mb-2">
                <span class="text-blue-700 font-medium flex items-center gap-2">
                    ...
                    <span id="scanProgressTitle">Memindai Jaringan Subnet {{ $activeSubnet }}.xxx...</span>
                </span>
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody id="ipTableBody" class="divide-y divide-gray-100">
                        @foreach($ipList as $item)
                            ...
                            @endphp
                            <tr id="row-ip-{{ $item['ip_suffix'] }}" class="hover:bg-blue-50/40 transition-colors {{ $rowBg }}"
                                data-suffix="{{ $item['ip_suffix'] }}"
                                data-ip="{{ $item['full_ip'] }}"
                                data-machine="{{ strtolower($item['excel_machine'] ?? '') }}"
                                data-user="{{ strtolower($item['excel_user'] ?? '') }}"
                                data-hostname="{{ strtolower($item['hostname'] ?? '') }}"
                                data-category="{{ $item['status_category'] }}">

                                <!-- IP -->
                                <td class="py-3 px-4 font-mono font-semibold text-gray-900">
                                    {{ $activeSubnet }}.<span class="text-blue-600 font-bold">{{ $item['ip_suffix'] }}</span>
                                </td>

                                <!-- Status -->
                                <td class="py-3 px-4 status-cell" id="status-cell-{{ $item['ip_suffix'] }}">
                                    @if($item['is_active'])
                                    ...
                                    @elseif($item['status_category'] === 'unknown')
                                ...
                                <!-- Hostname PC -->
                                <td class="py-3 px-4 hostname-cell" id="hostname-cell-{{ $item['ip_suffix'] }}">
                                    <span class="font-mono text-xs {{ $item['hostname'] ? 'text-indigo-700 bg-indigo-50 font-semibold px-2 py-0.5 rounded border border-indigo-100' : 'text-gray-400 italic' }}">
                                        {{ $item['hostname'] ?: '-' }}
                                    </span>
                                </td>
                                ...
                                <!-- Machine -->
                                <td class="py-3 px-4">
                                    <span class="font-medium {{ $item['excel_machine'] ? 'text-gray-900' : 'text-gray-400 italic' }}">
                                        {{ $item['excel_machine'] ?: '(Kosong)' }}
                                    </span>
                                </td>

                                <!-- User -->
                                <td class="py-3 px-4">
                                    <span class="font-medium {{ $item['excel_user'] ? 'text-gray-800' : 'text-gray-400 italic' }}">
                                        {{ $item['excel_user'] ?: '(Kosong)' }}
                                    </span>
                                </td>

                                <!-- OS / Perangkat -->
                                <td class="py-3 px-4 text-xs text-gray-600 device-cell">
                                    {{ $item['device_type'] !== 'Unknown' ? $item['device_type'] : '-' }}
                                </td>

                                <!-- Vendor (IEEE MAC) -->
                                <td class="py-3 px-4 text-xs vendor-cell" id="vendor-cell-{{ $item['ip_suffix'] }}">
                                    @if($item['vendor'])
                                        ...
                                        <div class="text-[11px] text-gray-500 mt-0.5">
                                            <i class="fa-solid fa-microchip text-blue-500 mr-1"></i> {{ $item['probable_device'] }}
                                        </div>
                                    @elseif($item['mac_address'])
                                        <span class="text-gray-400 text-[11px] font-mono" data-mac="{{ $item['mac_address'] }}" data-suffix="{{ $item['ip_suffix'] }}">Perangkat Jaringan</span>
                                    @else
                                ...
                                <!-- MAC Address -->
                                <td class="py-3 px-4 font-mono text-xs text-gray-500 mac-cell" id="mac-cell-{{ $item['ip_suffix'] }}">
                                    {{ $item['mac_address'] ?: '-' }}
                                </td>

                                <!-- Response Time -->
                                <td class="py-3 px-4 text-xs font-mono response-cell" id="resp-cell-{{ $item['ip_suffix'] }}">
                                    @if($item['response_time_ms'])
                                        <span class="text-green-700 font-semibold">{{ $item['response_time_ms'] }}ms</span>
                                    @else
                                ...
                                <td class="py-3 px-4 text-right space-x-1">
                                    <button onclick="diagnoseWithAi({{ $item['ip_suffix'] }})" id="row-btn-ai-{{ $item['ip_suffix'] }}" class="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-md transition" title="Diagnosa AI">
                                        <i class="fa-solid fa-robot"></i>
                                    </button>
                                    <button onclick="pingSingleIp({{ $item['ip_suffix'] }})" id="row-btn-ping-{{ $item['ip_suffix'] }}" class="p-1.5 text-green-600 hover:bg-green-50 rounded-md transition" title="Ping Saja">
                                        <i class="fa-solid fa-wifi"></i>
                                    </button>
                                    <button onclick="hostnameSingleIp({{ $item['ip_suffix'] }})" id="row-btn-hostname-{{ $item['ip_suffix'] }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md transition" title="Hostname + OS">
                                        <i class="fa-solid fa-desktop"></i>
                                    </button>
                                    <button onclick="macSingleIp({{ $item['ip_suffix'] }})" id="row-btn-mac-{{ $item['ip_suffix'] }}" class="p-1.5 text-purple-600 hover:bg-purple-50 rounded-md transition" title="MAC + Vendor">
                                        <i class="fa-solid fa-network-wired"></i>
                                    </button>
                                    <button onclick="openEditModal({{ $item['ip_suffix'] }}, '{{ addslashes($item['excel_machine'] ?? '') }}', '{{ addslashes($item['excel_user'] ?? '') }}', '{{ addslashes($item['excel_windows'] ?? '') }}')" class="p-1.5 text-gray-700 hover:bg-gray-100 rounded-md transition" title="Edit Excel">
        ...
        <div id="gridViewContainer" class="hidden grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            @foreach($ipList as $item)
                ...
                @endphp
                <div class="google-card p-4 flex flex-col justify-between {{ $cardBorder }}"
                    data-suffix="{{ $item['ip_suffix'] }}"
                    data-ip="{{ $item['full_ip'] }}"
                    data-machine="{{ strtolower($item['excel_machine'] ?? '') }}"
                    data-user="{{ strtolower($item['excel_user'] ?? '') }}"
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <span class="font-mono text-sm font-bold text-gray-900">
                            {{ $activeSubnet }}.<span class="text-blue-600 text-base">{{ $item['ip_suffix'] }}</span>
                        </span>
                        @if($item['is_active'])
                    ...
                    <div class="space-y-1 text-xs mb-3">
                        <div class="flex justify-between"><span class="text-gray-500">Hostname:</span> <span class="font-semibold text-indigo-700 font-mono">{{ $item['hostname'] ?: '-' }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Machine:</span> <span class="font-semibold text-gray-900">{{ $item['excel_machine'] ?: '(Kosong)' }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">User:</span> <span class="font-semibold text-gray-800">{{ $item['excel_user'] ?: '(Kosong)' }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">MAC:</span> <span class="font-mono text-gray-600">{{ $item['mac_address'] ?: '-' }}</span></div>
                        @if($item['response_time_ms'])
                            <div class="flex justify-between"><span class="text-gray-500">Response:</span> <span class="font-semibold text-green-700">{{ $item['response_time_ms'] }}ms</span></div>
                        @endif
                    </div>

                    <div class="flex items-center justify-end space-x-2 border-t border-gray-100 pt-2">
                        <button onclick="diagnoseWithAi({{ $item['ip_suffix'] }})" id="grid-btn-ai-{{ $item['ip_suffix'] }}" class="px-2 py-1 text-xs text-indigo-600 hover:bg-indigo-50 rounded-md font-medium" title="Diagnosa AI"><i class="fa-solid fa-robot"></i></button>
                        <button onclick="pingSingleIp({{ $item['ip_suffix'] }})" id="grid-btn-ping-{{ $item['ip_suffix'] }}" class="px-2 py-1 text-xs text-green-600 hover:bg-green-50 rounded-md font-medium" title="Ping"><i class="fa-solid fa-wifi"></i></button>
                        <button onclick="hostnameSingleIp({{ $item['ip_suffix'] }})" id="grid-btn-hostname-{{ $item['ip_suffix'] }}" class="px-2 py-1 text-xs text-blue-600 hover:bg-blue-50 rounded-md font-medium" title="Hostname"><i class="fa-solid fa-desktop"></i></button>
                        <button onclick="macSingleIp({{ $item['ip_suffix'] }})" id="grid-btn-mac-{{ $item['ip_suffix'] }}" class="px-2 py-1 text-xs text-purple-600 hover:bg-purple-50 rounded-md font-medium" title="MAC"><i class="fa-solid fa-network-wired"></i></button>
                        <button onclick="openEditModal({{ $item['ip_suffix'] }}, '{{ addslashes($item['excel_machine'] ?? '') }}', '{{ addslashes($item['excel_user'] ?? '') }}', '{{ addslashes($item['excel_windows'] ?? '') }}')" class="px-2.5 py-1 text-xs text-gray-700 hover:bg-gray-100 rounded-md font-medium">Edit</button>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl space-y-4 border border-gray-200">
            <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                <h3 class="text-base font-bold text-gray-900 flex items-center gap-2">
                    <i class="fa-solid fa-file-excel text-green-600"></i> Update Excel — IP <span id="modalIpDisplay" class="text-blue-600">{{ $activeSubnet }}.X</span>
                </h3>
        ...
        <div class="bg-white rounded-2xl max-w-lg w-full shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div id="scanModalHeader" class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                <h3 class="text-base font-bold text-gray-900 flex items-center gap-2">
                    ...
                    <span id="scanModalIp" class="text-blue-600">{{ $activeSubnet }}.X</span>
                </h3>
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div id="scanLoading" class="hidden flex flex-col items-center justify-center py-8">
                    <div class="flex items-center gap-3 text-gray-600 mb-3">
                        ...
                        <span class="text-sm font-medium">Memindai {{ $activeSubnet }}.<span id="scanLoadingSuffix">X</span>...</span>
                    </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-white/80 flex items-center gap-1.5">
                        ...
                        <span>Subnet {{ $activeSubnet }}.x &bull; Tool-Calling Active</span>
                    </p>
                </div>
            </div>
    ...
    <script>
        ...
        async function pingSingleIp(suffix) {
            ...
            try {
                const res = await fetch('{{ route("ip.scan.ping") }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                    body: JSON.stringify({ ip_suffix: suffix, subnet: '{{ $activeSubnet }}' })
                });
        ...
        async function hostnameSingleIp(suffix) {
            ...
            if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; }
            try {
                const res = await fetch('{{ route("ip.scan.hostname") }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                    body: JSON.stringify({ ip_suffix: suffix, subnet: '{{ $activeSubnet }}' })
                });
        ...
        async function macSingleIp(suffix) {
            ...
            if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; }
            try {
                const res = await fetch('{{ route("ip.scan.mac") }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                    body: JSON.stringify({ ip_suffix: suffix, subnet: '{{ $activeSubnet }}' })
                });
            ...
            try {
                for (let i = start; i <= end; i += CHUNK) {
                    ...
                    for (let j = i; j <= chunkEnd; j++) {
                        promises.push(fetch('{{ route("ip.scan.ping") }}', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                            body: JSON.stringify({ ip_suffix: j, subnet: '{{ $activeSubnet }}' })
        ...
        async function startBatchHostname(start, end) {
            ...
            try {
                for (let i = start; i <= end; i += CHUNK) {
                    ...
                    const promises = [];
                    for (let j = i; j <= chunkEnd; j++) {
                        promises.push(fetch('{{ route("ip.scan.hostname") }}', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                            body: JSON.stringify({ ip_suffix: j, subnet: '{{ $activeSubnet }}' })
                        }).then(r => r.json()).then(data => {
        ...
        async function startBatchMac(start, end) {
            ...
            try {
                for (let i = start; i <= end; i += CHUNK) {
                    ...
                    const promises = [];
                    for (let j = i; j <= chunkEnd; j++) {
                        promises.push(fetch('{{ route("ip.scan.mac") }}', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                            body: JSON.stringify({ ip_suffix: j, subnet: '{{ $activeSubnet }}' })
                        }).then(r => r.json()).then(data => {
            ...
            try {
                ...
                for (let ci = 0; ci < totalChunks; ci++) {
                    ...
                    const res = await fetch('/scan/range', {
                        ...
                        body: JSON.stringify({ start: chunkStart, end: chunkEnd, subnet: '{{ $activeSubnet }}' })
                    });
                    ...
                    progressDetail.innerText = `{{ $activeSubnet }}.${chunkStart} - .${chunkEnd} | ${totalActive} aktif`;
                }
        ...
        function openEditModal(suffix, machine, user, windows) {
            ...
            document.getElementById('modalIpDisplay').innerText = `{{ $activeSubnet }}.${suffix}`;
        ...
        async function saveExcelData(e) {
            ...
            try {
                const res = await fetch('/excel/update', {
                    ...
                    body: JSON.stringify({ ip_suffix: suffix, machine, user, windows, subnet: '{{ $activeSubnet }}' })
                });
        ...
        // ═══════════════════════════════════════════════════════
        const activeSubnet = '{{ $activeSubnet }}';
